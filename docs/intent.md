...
- Catalog APIs validate JWTs via **local signature verification** using **JWKS from auth over HTTP** (one DB per service; catalog does **not** read auth-db and does **not** call `/validate`). **Redis** is catalog cache + latest PDF + **locks**.
