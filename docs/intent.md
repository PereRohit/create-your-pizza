...
- PDF artifacts are stored **in the database as version history**. **Redis holds the latest menu only**; past versions always hit Postgres. Dirty/5-min job + `pdf_generation` lock (admin writes → 503) remain in force; the job does **not** skip because a catalog save is in progress.
...
- PDF generation runs **asynchronously every 5 minutes**, and **only when the catalog is dirty**, with **`pdf_generation` lock** during generation (admin writes → **HTTP 503** retry). No `catalog_write` skip.
...
- **PDF:** async, **5-minute** cadence, dirty-only; DB **history**; Redis **latest**; `pdf_generation` → 503 on writes; no catalog-write skip
